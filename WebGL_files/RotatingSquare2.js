 ...
 * <pre>
 *   let m = new Matrix4();                           // identity matrix
 *   m.setTranslate(0.3, 0.0, 0.0);                   // make it into a translation matrix
 *   let m2 = new Matrix4().setRotate(90, 0, 0, 1);   // create and make rotation in one step
 *                                                    // (rotate 90 degrees in x-y plane)
 *   m.multiply(m2);                                  // multiply m on right by m2, i.e., m = m * m2;
 *   let theRealData = m.elements;                    // get the underlying Float32Array
 *                                                       (this part is sent to shader)
 *
 *   Alternatively, can chain up the operations:
 *
 *   let m = new Matrix4().setTranslate(0.3, 0.0, 0.0).rotate(90, 0, 0, 1);
 * </pre>
 *
 * @author Paulo Roma
 * @since 27/09/2016
 ...
 * @type {Matrix4}
 */
let modelMatrix = new Matrix4(); // identity matrix
 ...
 * @type {Matrix4}
 */
let projectionMatrix = new Matrix4().setOrtho(
  -wsize / 2,
  wsize / 2,
  -wsize / 2,
  wsize / 2,
...
function draw() {
  ...
  let colorIndex = gl.getAttribLocation(shader, "a_Color");
  if (colorIndex < 0) {
  ...
  let transformLoc = gl.getUniformLocation(shader, "transform");
 ...
 */
 function mainEntrance() {
  ...
  let vshaderSource = document.getElementById("vertexShader").textContent;
  let fshaderSource = document.getElementById("fragmentShader").textContent;
  ...
  let runanimation = (() => {
    ...
    let corner = new Vector4([...getVertex(cindex), 0.0, 1.0]);

    // vertex of each color
    let vr     = new Vector4([...getVertex(0), 0.0, 1.0]);
    let vg     = new Vector4([...getVertex(1), 0.0, 1.0]);
    let vb     = new Vector4([...getVertex(2), 0.0, 1.0]);
    let vw     = new Vector4([...getVertex(5), 0.0, 1.0]);
    ...
    return () => {
      ...
      // updates position of each vertex of the square
        vr.elements[0] = getVertex(0)[0];
        vr.elements[1] = getVertex(0)[1];
        vr = modelMatrix.multiplyVector4(vr);

        vg.elements[0] = getVertex(1)[0];
        vg.elements[1] = getVertex(1)[1];
        vg = modelMatrix.multiplyVector4(vg);

        vb.elements[0] = getVertex(2)[0];
        vb.elements[1] = getVertex(2)[1];
        vb = modelMatrix.multiplyVector4(vb);

        vw.elements[0] = getVertex(5)[0];
        vw.elements[1] = getVertex(5)[1];
        vw = modelMatrix.multiplyVector4(vw);

       function test(){
          let vcolor = [vr, vg, vb, vw];
          vcolor.forEach((item, i) => {
            if(i == 3) i = 5;
            if (  click
               ||item.elements[0] < -2.5
               || item.elements[1] < -2.5
